...
async function loadSharedPlans(){
  if(syncReady || !config.appsScriptUrl || !config.sharedKey) return;
  syncReady = true;
  ...
  Object.entries(local).forEach(([key, plan]) => {
    if(!remote[key] || planTimestamp(plan) > planTimestamp(remote[key])){
      merged[key] = plan;
...
async function loadRecipes(){
  ...
  try{
    ...
    try{
      ...
      $("weekStatus").textContent = `Using this device's saved plans: ${syncError.message}`;
    }
    renderPlanner();
    renderResults();
  }catch(error){
