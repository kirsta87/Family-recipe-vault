...
async function buildReviewFromVisualMapper(){const s=visualMapperState,btn=$("mapperBuildReview");btn.disabled=true;const candidates=[];let done=0;for(const page of s.pages){if(s.skippedPages.has(page.page))continue;const groups=mapperPageGroups(page.page);for(const [gi,g] of groups.entries()){const allBoxes=groups.flatMap(x=>Object.values(x.boxes||{}).filter(Boolean));const title=mappedText(page,g.boxes.title,"title",allBoxes).replace(/\s+/g," ").trim();const ingredients=mappedText(page,g.boxes.ingredients,"ingredients",allBoxes);const instructions=mappedText(page,g.boxes.instructions,"instructions",allBoxes);if(!title&&!ingredients.length&&!instructions.length)continue;const nutrition=mappedText(page,g.boxes.nutrition,"prose",allBoxes).replace(/\n+/g," | ");const yieldText=mappedText(page,g.boxes.yieldText,"title",allBoxes);const description=mappedText(page,g.boxes.description,"prose",allBoxes).replace(/\n+/g," ");const image=await cropMappedImage(s.pdf,page.page,g.boxes.image).catch(()=>"");const warnings=[];if(!title)warnings.push("Missing title");if(!ingredients.length)warnings.push("Missing ingredients");if(!instructions.length)warnings.push("Missing instructions");candidates.push({page:page.page,endPage:page.page,title:title||`Page ${page.page} recipe ${gi+1}`,titleSource:"visual mapper",titleConfidence:title?100:25,ingredients,instructions,nutrition,yieldText,description,image,imageKind:image?"mapped crop":"",useImage:Boolean(image),links:page.links||[],warnings,include:true,protein:"",type:"",cuisine:"",importStatus:"new",pageWidth:page.width,pageHeight:page.height,debugReport:{source:"Visual Cookbook Mapper v2",page:page.page,group:gi+1,boxes:g.boxes}});done++;btn.textContent=`Building ${done}…`;await nextFrame();}}importState.candidates=candidates;$("visualMapperPanel").hidden=true;btn.disabled=false;btn.textContent="Build recipe review";showReview();}
