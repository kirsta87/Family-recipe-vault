...
function duplicateComparisonMarkup(candidate,index){
  ...
  return `<button type="button" class="secondary compact-button duplicate-compare-button" data-toggle-duplicate-compare="${index}">Compare with saved recipe</button><section id="duplicate-compare-${index}" class="duplicate-comparison" hidden><div class="duplicate-compare-column"><h4>This cookbook recipe</h4>${image(candidateImage)}<strong>${escapeHTML(candidate.title||"Untitled recipe")}</strong><h5>Ingredients</h5>${list(candidate.ingredients||[])}<h5>Instructions</h5>${list(candidate.instructions||[])}</div><div class="duplicate-compare-column"><h4>Already in your vault</h4>${image(existingImage)}<strong>${escapeHTML(existing.name||"Untitled recipe")}</strong><h5>Ingredients</h5>${list(existingIngredients)}<h5>Instructions</h5>${list(existingInstructions)}</div><div class="duplicate-decision-row"><button type="button" class="secondary compact-button ${action==="skip"?"is-selected":""}" data-duplicate-action="skip" data-duplicate-index="${index}">Skip</button><button type="button" class="secondary compact-button ${action==="keep"?"is-selected":""}" data-duplicate-action="keep" data-duplicate-index="${index}">Keep both</button><button type="button" class="primary compact-button ${action==="replace"?"is-selected":""}" data-duplicate-action="replace" data-duplicate-index="${index}">Replace saved recipe</button></div></section>`;
}
...
async function importSelected(){
  ...
  $("reviewPanel").hidden=true;$("importResults").hidden=false;$("importResults").innerHTML=`<div class="success-panel"><div class="success-icon">✓</div><h2>${escapeHTML(title)} is on your shelf</h2><p>${imported} recipes imported${replaced?`, ${replaced} saved recipes replaced`:""}${skipped?`, ${skipped} duplicates skipped`:""}${previewsAttached?`, ${previewsAttached} original PDF pages attached`:""}${failed?`, ${failed} failed`:""}.</p><div class="actions"><button id="browseImported" class="primary">Browse cookbook</button><a class="secondary linkbtn" href="index.html">Return to Recipe Vault</a></div></div>`;$("browseImported").onclick=()=>{loadRecipes().then(()=>openCookbook(id));};
}
