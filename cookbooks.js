...
/* Build 204 — Visual Cookbook Mapper editing tools */
...
function mapperRefreshToolbar(){
 ...
 const tip=$("mapperInstruction");if(tip)tip.textContent=`Draw ${MAPPER_LABELS[s.activeField]} for ${groupSel?.selectedOptions?.[0]?.textContent||"Recipe 1"}. Click a box to move or resize it.`;
...
async function launchVisualCookbookMapper(pdf,pages,tocMap){
 ...
 panel.innerHTML=`<div class="mapper-header"><div><p class="eyebrow">BUILD 206 · COOKBOOK UPLOAD FIX</p><h2>Show Recipe Vault where everything lives</h2><p class="muted">Draw the usual layout once, then move or resize any box. Extra recipes and individual page changes stay on that page.</p></div><button id="mapperCancel" class="secondary">Cancel</button></div><div class="mapper-toolbar"><label>Field<select id="mapperFieldSelect">${MAPPER_FIELDS.map(f=>`<option value="${f}">${MAPPER_LABELS[f]}</option>`).join("")}</select></label><label>Recipe<select id="mapperGroupSelect"></select></label><button id="mapperAddGroup" class="secondary">+ Add recipe to page</button><button id="mapperDuplicateGroup" class="secondary">Duplicate recipe layout</button><label class="mapper-toggle"><input id="mapperPageOnly" type="checkbox"> Edit this page only</label><button id="mapperResetPage" class="secondary">Reset page to template</button><button id="mapperBuildReview" class="primary">Build recipe review</button></div><div class="mapper-status"><strong id="mapperModeBadge"></strong><span id="mapperInstruction"></span></div><div id="mapperPages" class="mapper-pages"></div>`;
 ...
 try{
  ...
  for(const page of includedPages){
   ...
   for(const [gi,g] of groups.entries()){
    ...
    candidates.push({page:page.page,endPage:page.page,title:title||`Page ${page.page} recipe ${gi+1}`,titleSource:"visual mapper",titleConfidence:title?100:25,ingredients,instructions,nutrition,yieldText,description,image:"",imageKind:g.boxes.image?"mapped crop pending":"",useImage:Boolean(g.boxes.image),mappedImageBox:g.boxes.image?{...g.boxes.image}:null,links:page.links||[],warnings,include:true,protein:"",type:"",cuisine:"",importStatus:"new",pageWidth:page.width,pageHeight:page.height,debugReport:{source:"Visual Cookbook Mapper v4",page:page.page,group:gi+1,boxes:JSON.parse(JSON.stringify(g.boxes||{}))}});
