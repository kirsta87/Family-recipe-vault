...
function classifyImportCandidates(){
  ...
  importState.candidates.forEach(r=>{
    ...
    if(best&&bestScore>=.84){
      const bestBookId=String(recipeField(best,"cookbook_id","cookbookId")||"").trim();
      const targetBookId=String(matched?.id||"").trim();
      const belongsElsewhere=Boolean(bestBookId&&(!targetBookId||bestBookId!==targetBookId));
      r.importStatus="possible-duplicate";
      r.duplicateMatch=best;
      r.duplicateConfidence=bestScore;
      r.duplicateBelongsElsewhere=belongsElsewhere;
      // The uploaded PDF is the source of truth. A strong match currently stored
      // under another cookbook is queued to be reclaimed, but nothing is written
      // until the user confirms the whole import batch.
      r.duplicateAction=belongsElsewhere?"reclaim":"skip";
      r.include=belongsElsewhere;
    }else r.include=true;
  });
}
...
function duplicateComparisonMarkup(candidate,index){
  ...
  const reclaimNote=candidate.duplicateBelongsElsewhere?`<p class="muted duplicate-reclaim-note">This saved recipe is currently attached to ${escapeHTML(recipeCookbookLabel(existing)||"another cookbook")}. The uploaded PDF can reclaim it without creating a second copy.</p>`:"";
  return `<button type="button" class="secondary compact-button duplicate-compare-button" data-toggle-duplicate-compare="${index}">Compare with saved recipe</button><section id="duplicate-compare-${index}" class="duplicate-comparison" hidden>${reclaimNote}<div class="duplicate-compare-column"><h4>This cookbook recipe</h4>${image(candidateImage)}<strong>${escapeHTML(candidate.title||"Untitled recipe")}</strong><h5>Ingredients</h5>${list(candidate.ingredients||[])}<h5>Instructions</h5>${list(candidate.instructions||[])}</div><div class="duplicate-compare-column"><h4>Already in your vault</h4>${image(existingImage)}<strong>${escapeHTML(existing.name||"Untitled recipe")}</strong><h5>Ingredients</h5>${list(existingIngredients)}<h5>Instructions</h5>${list(existingInstructions)}</div><div class="duplicate-decision-row">${candidate.duplicateBelongsElsewhere?`<button type="button" class="primary compact-button ${action==="reclaim"?"is-selected":""}" data-duplicate-action="reclaim" data-duplicate-index="${index}">Move existing recipe here</button>`:""}<button type="button" class="secondary compact-button ${action==="skip"?"is-selected":""}" data-duplicate-action="skip" data-duplicate-index="${index}">Skip</button><button type="button" class="secondary compact-button ${action==="keep"?"is-selected":""}" data-duplicate-action="keep" data-duplicate-index="${index}">Keep both</button>${candidate.duplicateBelongsElsewhere?"":`<button type="button" class="primary compact-button ${action==="replace"?"is-selected":""}" data-duplicate-action="replace" data-duplicate-index="${index}">Replace saved recipe</button>`}</div></section>`;
}
...
async function importSelected(){
  ...
  let imported=0,replaced=0,reclaimed=0,skipped=0,failed=0,previewsAttached=0,completed=0;
  ...
  const runImport=async r=>{
    try{
      ...
      if(r.importStatus==="possible-duplicate"&&(r.duplicateAction==="replace"||r.duplicateAction==="reclaim")&&r.duplicateMatch){
        ...
        Object.assign(existing,updates);if(r.duplicateAction==="reclaim")reclaimed++;else replaced++;
      }else{
  ...
  // Rebuild ownership indexes after confirmed reclaim operations so the recipe
  // disappears from the wrong cookbook and appears only in this one.
  library.forEach(book=>{book.recipeRefs=recipes.filter(recipe=>String(recipeField(recipe,"cookbook_id","cookbookId")||"")===String(book.id||"")).map(recipeStableRef);book.importedCount=book.recipeRefs.length;});
  saveLibrary();btn.disabled=false;btn.textContent="Import selected";
  $("reviewPanel").hidden=true;$("importResults").hidden=false;$("importResults").innerHTML=`<div class="success-panel"><div class="success-icon">✓</div><h2>${escapeHTML(title)} is on your shelf</h2><p>${imported} recipes imported${reclaimed?`, ${reclaimed} misplaced recipes moved here`:""}${replaced?`, ${replaced} saved recipes replaced`:""}${skipped?`, ${skipped} duplicates skipped`:""}${previewsAttached?`, ${previewsAttached} original PDF pages attached`:""}${failed?`, ${failed} failed`:""}.</p><div class="actions"><button id="browseImported" class="primary">Browse cookbook</button><a class="secondary linkbtn" href="index.html">Return to Recipe Vault</a></div></div>`;$("browseImported").onclick=()=>{loadRecipes().then(()=>openCookbook(id));};
}
