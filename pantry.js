...
function render(){
  ...
  $("pantryItems").innerHTML=visible.length?visible.map(item=>`<article class="pantry-item-card ${bulkEditMode?"is-bulk-editing":""}" data-edit-pantry="${escapeHTML(item.id)}">
    ...
    <div class="pantry-item-main"><h3>${escapeHTML(item.name)}</h3>${item.brand||item.store?`<p class="pantry-brand-line">${escapeHTML([item.brand,item.store].filter(Boolean).join(" · "))}</p>`:""}<p class="pantry-quantity-line"><strong>${escapeHTML([item.amount,item.unit].filter(Boolean).join(" ")||"On hand")}</strong><span>${escapeHTML(item.status==="unopened"?"New / unopened":item.status==="frozen"?"Frozen":item.status==="use-soon"?"Use soon":"Open")}</span></p></div>
    <div class="pantry-item-meta"><span class="pantry-mode-badge">${escapeHTML(modeLabel(item.mode))}</span>${item.mode==="stocked"&&item.targetStock?`<small>Keep ${escapeHTML(item.targetStock)} on hand</small>`:""}${item.packageSize?`<small>${escapeHTML(item.packageSize)} ${escapeHTML(item.packageUnit||"")} package</small>`:""}${item.source?`<small>${escapeHTML(item.source==="shopping"?"Added from shopping":item.source.startsWith("meal:")?`Updated by ${item.source.slice(5)}`:item.source)}</small>`:""}</div>
