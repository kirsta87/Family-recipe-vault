...
function assignRecipeToDate(recipe, date){
  ...
  localStorage.setItem(PLANNER_KEY,JSON.stringify(slot.plans));
  planner=slot.plans;
  return true;
}
function renderMealPlanDateChoices(){
  const start=plannerMonday(new Date());
  $("mealPlanDateChoices").innerHTML=Array.from({length:14},(_,index)=>{
    const date=plannerAddDays(start,index), slot=plannerSlot(date), existing=slot.recipeId ? plannerRecipeName(slot.recipeId) : "Empty";
    return `<button class="meal-plan-date-choice ${slot.recipeId ? "occupied" : ""}" type="button" data-meal-plan-date="${date.toISOString().slice(0,10)}">
...
document.addEventListener("click",event=>{
  ...
  if(dateButton && mealPlanRecipe){
    ...
    if(assignRecipeToDate(mealPlanRecipe,date)){
      $("mealPlanDialogStatus").textContent=`Added to ${plannerDateLabel(date)}.`;
      $("mealPlanDialogStatus").className="import-status success";
      renderMealPlanDateChoices();
    }
  }
});
