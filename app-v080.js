...
function searchScore(recipe, query){
  ...
  const buckets = [
    ...
    {weight:60, label:"category match", values:[recipe.protein, recipe.type, recipe.cuisine, ...(recipe.tags || [])]},
...
function render(){
  ...
  document.querySelectorAll("[data-id]").forEach(button => {
    button.addEventListener("click", () => {
      openRecipe(
        recipes.find(recipe => recipe.id === button.dataset.id)
      );
    });
  });
}
...
async function write(action, recipe, updates){
  ...
  try{
    ...
    await setupNewCategory("manualProtein", "manualProteinNew");
