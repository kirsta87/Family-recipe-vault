...
async function loadSharedPlans(force = false){
  ...
  const remote = Object.fromEntries(Object.entries(rawRemote).map(([key, plan]) => [key, normalizePlanShape(plan, key)]));
  ...
  Object.entries(local).forEach(([key, plan]) => {
    ...
    const localHasContent = planHasContent(plan);
    const remoteHasContent = planHasContent(remotePlan);

    // Migrate real browser plans over placeholder/empty shared week rows.
    if(localHasContent && (!remotePlan || !remoteHasContent)){
      merged[key] = plan;
      plansToUpload.push([key, plan]);
    }
  });
...
// Shopping list ------------------------------------------------------------
const SHOPPING_CATEGORIES = ["Produce","Meat & Seafood","Dairy & Eggs","Bakery","Frozen","Pantry","Other"];
...
function categoryForIngredient(text){
  ...
  if(/\b(chicken|beef|steak|pork|bacon|sausage|turkey|salmon|shrimp|fish|ground meat)\b/.test(value)) return "Meat & Seafood";
  if(/\b(milk|cream|cheese|parmesan|mozzarella|butter|yogurt|egg|eggs|sour cream)\b/.test(value)) return "Dairy & Eggs";
  if(/\b(bread|bun|buns|tortilla|tortillas|pita|naan|roll|rolls|bagel)\b/.test(value)) return "Bakery";
  if(/\b(frozen|ice cream)\b/.test(value)) return "Frozen";
  if(/\b(onion|garlic|pepper|tomato|potato|carrot|celery|lettuce|spinach|broccoli|lime|lemon|avocado|cilantro|parsley|mushroom|zucchini|cucumber|corn|ginger|apple|mango|scallion|green onion)\b/.test(value)) return "Produce";
  if(/\b(rice|pasta|noodle|flour|sugar|oil|vinegar|broth|stock|sauce|spice|seasoning|salt|pepper|beans|lentils|can|canned|breadcrumbs|honey|mustard|mayonnaise|mayo|ketchup)\b/.test(value)) return "Pantry";
  return "Other";
}
...
function parseIngredientLine(line){
  ...
  const match = text.match(/^(?:(\d+(?:\.\d+)?)\s+)?(\d+\/\d+|\d+(?:\.\d+)?)(?:\s+)(cups?|c|tablespoons?|tbsp|teaspoons?|tsp|ounces?|oz|pounds?|lbs?|lb|grams?|g|kilograms?|kg|milliliters?|ml|liters?|l|cloves?|cans?|packages?|pkg|sticks?|slices?|pieces?)\b\s*(.*)$/i);
  if(match){
    ...
    if(Number.isFinite(amount) && name){
      return {original, amount, unit, name, key:`${unit}|${name.toLowerCase().replace(/[^a-z0-9]+/g," ").trim()}`};
    }
  }
  const noUnit = text.match(/^(?:(\d+(?:\.\d+)?)\s+)?(\d+\/\d+|\d+(?:\.\d+)?)\s+(.+)$/);
  if(noUnit){
    const amount = (noUnit[1] ? Number(noUnit[1]) : 0) + numberFromToken(noUnit[2]);
    const name = noUnit[3].trim();
    if(Number.isFinite(amount) && name){
      return {original, amount, unit:"", name, key:`|${name.toLowerCase().replace(/[^a-z0-9]+/g," ").trim()}`};
    }
  }
  return {original, amount:null, unit:"", name:original, key:`raw|${original.toLowerCase().replace(/\s+/g," ")}`};
}
...
function selectedShoppingRecipes(){
  return [...document.querySelectorAll('[data-shopping-recipe]:checked')]
...
function openShoppingListDialog(){
  ...
  $("shoppingListIntro").textContent = planned.length
    ? "Uncheck any planned meal you are not shopping for this week."
    : "Add meals to this week before generating a shopping list.";
  ...
  $("generateShoppingList").hidden = !planned.length;
...
function buildShoppingList(){
  ...
  [...merged.values()].forEach(item => {
    const display = item.amount === null ? item.original : `${prettyAmount(item.amount)}${item.unit ? ` ${item.unit}` : ""} ${item.name}`;
    grouped[categoryForIngredient(item.name)].push(display.trim());
  });
  const activeGroups = SHOPPING_CATEGORIES.filter(name => grouped[name].length);
  $("shoppingListOutput").innerHTML = `
    <div class="shopping-list-summary"><strong>${merged.size} ingredient${merged.size === 1 ? "" : "s"}</strong><span>From ${selected.length} recipe${selected.length === 1 ? "" : "s"}</span></div>
    ${activeGroups.map(group => `<section class="shopping-category"><h3>${escapeHTML(group)}</h3>${grouped[group].sort((a,b)=>a.localeCompare(b)).map(item => `<label class="shopping-item"><input type="checkbox"><span>${escapeHTML(item)}</span></label>`).join("")}</section>`).join("")}
    <section class="shopping-list-recipes"><h3>Generated from</h3><p>${selected.map(recipe => escapeHTML(recipe.name || "Untitled recipe")).join(" • ")}</p></section>`;
  latestShoppingText = activeGroups.map(group => `${group}\n${grouped[group].sort((a,b)=>a.localeCompare(b)).map(item => `☐ ${item}`).join("\n")}`).join("\n\n") + `\n\nGenerated from: ${selected.map(recipe => recipe.name || "Untitled recipe").join(", ")}`;
  ...
  $("shoppingListStatus").textContent = "Matching ingredients were combined when their name and unit matched.";
