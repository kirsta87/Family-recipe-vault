(() => {
  ...
  const assetBuild = Number(window.RECIPE_VAULT_BUILD || 0);
  const scriptBuild = Number(scriptUrl.searchParams.get("v") || 0);
  const CURRENT_BUILD = Math.max(assetBuild, metaBuild || 0, scriptBuild || 0, 181);
  ...
  style.textContent = `
    ...
    #rv-build-panel[hidden]{display:none} #rv-build-panel strong{display:block;margin-bottom:4px} #rv-build-panel p{margin:5px 0;color:#62665d} #rv-build-panel button{margin-top:10px;width:100%;border:0;border-radius:10px;padding:10px 12px;background:#667653;color:white;font-weight:700;cursor:pointer} #rv-build-panel button.secondary{background:#eceee7;color:#394033}
  `;
  ...
  async function clearAndReload(){
    ...
    const button=document.getElementById("rv-refresh-latest");
    button.disabled=true; msg.textContent="Clearing old app files…";
    const failures=[];
    if("serviceWorker" in navigator){
      try{
        const regs=await navigator.serviceWorker.getRegistrations();
        const scoped=regs.filter(r=>!r.scope||r.scope.startsWith(location.origin));
        const results=await Promise.allSettled(scoped.map(r=>r.unregister()));
        results.forEach(r=>{if(r.status==="rejected")failures.push(r.reason);});
      }catch(err){failures.push(err);}
    }
    if("caches" in window){
      try{
        const keys=await caches.keys();
        const results=await Promise.allSettled(keys.filter(k=>k.startsWith("recipe-vault-")).map(k=>caches.delete(k)));
        results.forEach(r=>{if(r.status==="rejected")failures.push(r.reason);});
      }catch(err){failures.push(err);}
    }
    try{sessionStorage.removeItem("recipeVaultUpdatePending");}catch(_e){}
    if(failures.length)console.warn("Recipe Vault cache cleanup completed with warnings:",failures);
    msg.textContent="Opening a fresh copy…";
    const url=new URL(location.href);
    url.searchParams.set("rv_build",String(liveBuild?.build||Date.now()));
    url.searchParams.set("rv_fresh",String(Date.now()));
  ...
  async function check(){
    ...
    try{
      ...
      if(deployed>CURRENT_BUILD){
        ...
        badge.dataset.state="current"; badge.textContent=deployed===CURRENT_BUILD?`Build ${CURRENT_BUILD} · Up to date`:`Build ${CURRENT_BUILD} · Loaded`;
        msg.textContent=deployed===CURRENT_BUILD?"The loaded page matches the latest deployed build.":`Build ${CURRENT_BUILD} is loaded. The deployment version file has not caught up yet.`;
      }
    }catch(err){
