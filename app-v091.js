...
function refreshEntryCategoryMenus(){
  ...
  fillCategorySelect("manualCollection", collections);
}
...
function renderInlineEditor(recipe){
  ...
  return `
    <form class="card-inline-editor" data-inline-edit-id="${safeId}">
      ...
      <label class="inline-edit-field inline-edit-title">Title
      ...
      <div class="inline-edit-grid">
        ...
        <label class="inline-edit-field">Collection
          <select name="collection">${inlineSelectOptions(collectionValues(), collection)}</select>
          <input class="inline-new-value" name="collectionNew" type="text" placeholder="New collection" hidden>
        </label>
      </div>
...
function render(){
  ...
  $("grid").classList.toggle("inline-edit-mode", editMode);
...
document.addEventListener("submit", async event => {
  ...
  const updates = {
    ...
    collections: inlineSelectedValue(form, "collection", "collectionNew")
  };
...
function openQuickEdit(recipe){
  ...
  $("quickEditCollectionNew").hidden =
    $("quickEditCollection").value !== "__new__";
...
on("editModeBtn", "click", () => {
  editMode = !editMode;
  const button = $("editModeBtn");
  button.textContent = editMode ? "Done editing" : "Edit recipes";
  button.setAttribute("aria-pressed", String(editMode));
  button.classList.toggle("primary", editMode);
  button.classList.toggle("secondary", !editMode);
  render();
});
...
on("manualRecipeForm", "submit", async event => {
  ...
  try{
    ...
    $("manualCollectionNew").hidden = true;
    refreshEntryCategoryMenus();
...
function populateRecipeEditor(recipe){
  ...
  fillCategorySelect("editCollection", collections, (recipe.collections || [])[0] || "");
  ...
  $("editCollectionNew").value = "";
  $("editCollectionNew").hidden = $("editCollection").value !== "__new__";
