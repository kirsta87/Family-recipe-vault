...
function renderPool(){
  ...
  $("recipePool").innerHTML = poolRecipes.length ? poolRecipes.map(recipe => {
    ...
    return `<article class="recipe-pool-card ${assignedDay ? "assigned" : ""}">
      ...
      <div class="recipe-pool-content">
        ...
        ${assignedDay ? `<p class="pool-assigned-note">Scheduled for ${escapeHTML(assignedDay)}, ${escapeHTML(formatDate(addDays(activeWeek, days.indexOf(assignedDay))))}</p>` : '<p class="pool-unscheduled-note">Not assigned to a day yet</p>'}
        <div class="pool-card-actions">
          <button class="primary compact" type="button" data-assign-recipe="${escapeHTML(recipe.id)}">${assignedDay ? "Move or copy to day" : "Assign to day"}</button>
  ...
  $("plannerResults").innerHTML = visible.map(recipe => {
    ...
    return `<article class="planner-result-card">
      ...
      <div>
        ...
        ${assignedDay ? `<p class="planner-result-scheduled">Scheduled ${escapeHTML(assignedDay)}</p>` : ""}
  ...
  return `<section class="meal-plan-week-group">
    ...
    <div class="meal-plan-week-days">
      ${days.map((day, index) => {
        ...
        return `<button class="meal-plan-date-choice ${recipeId ? "occupied" : ""}" type="button" data-assign-date="${date.toISOString().slice(0,10)}">
          <strong>${escapeHTML(fullDate(date))}</strong><span>${escapeHTML(existing)}</span>
        </button>`;
      }).join("")}
    </div>
  </section>`;
