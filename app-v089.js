  ...
  $("grid").innerHTML = visible.map(item => {
    ...
    return `
      <article class="card" data-id="${escapeHTML(recipe.id)}" role="button" tabindex="0" aria-label="Open ${escapeHTML(recipe.name || "recipe")}">
        <button class="card-quick-edit" type="button" data-quick-edit-id="${escapeHTML(recipe.id)}" aria-label="Quick edit ${escapeHTML(recipe.name || "recipe")}"${editMode ? "" : " hidden"}>Edit</button>
