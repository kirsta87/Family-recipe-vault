...
on("addForm", "submit", async event => {
  ...
  try{
    ...
    alert(`Imported ${result.recipe?.name || "recipe"}.`);
  }catch(error){
    alert(`Could not import recipe: ${error.message}`);
  }
});
