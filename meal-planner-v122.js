...
async function loadSharedPlans(force = false){
  ...
  Object.entries(local).forEach(([key, localPlan]) => {
    ...
    // Only a plan explicitly marked pending may override shared data.
    // This prevents an older browser copy from resurrecting a deleted meal.
    ...
    // One-time migration for a populated browser week that does not exist remotely.
    if(!remotePlan && planHasContent(localPlan)){
      localPlan.pendingSync = true;
      merged[key] = localPlan;
      plansToUpload.push([key, localPlan]);
    }
  });
