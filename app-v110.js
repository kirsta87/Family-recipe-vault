...
async function plannerPost(payload){
  if(!config.appsScriptUrl || !config.sharedKey) return null;
...
async function loadSharedPlanner(){
  ...
  try{
    ...
    Object.entries(local).forEach(([key, plan]) => {
      if(!remote[key] || plannerTimestamp(plan) > plannerTimestamp(remote[key])){
        merged[key] = plan;
        localNewer.push([key, plan]);
      }
    });
    planner = merged;
    localStorage.setItem(PLANNER_KEY, JSON.stringify(merged));
    for(const [key, plan] of localNewer){
